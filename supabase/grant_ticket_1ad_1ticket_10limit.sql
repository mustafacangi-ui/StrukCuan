...
-- Fixes UUID/TEXT type mismatch from previous versions.
-- Column types:
--   ad_ticket_events.user_id  = UUID  → use uid (no cast)
--   user_stats.user_id        = TEXT  → use uid::text
...
begin
  ...
  -- Count today's rewarded ads (ad_ticket_events.user_id is UUID)
  ...
  -- Record the ad event (user_id UUID column → pass uid directly)
  insert into public.ad_ticket_events (user_id, event_type, week_id)
  values (uid, 'rewarded', v_date_id);

  -- +1 ticket to user_stats (user_id TEXT column → cast)
  ...
  -- (user_tickets & lottery_tickets both have TEXT user_id → cast)
  if coalesce(
